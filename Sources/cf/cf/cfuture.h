...
class default_executor {
    struct any_executor {
        virtual void post(cf::detail::task_type task) = 0;
        virtual ~any_executor() { }
    };
    
    template<typename E>
    struct template_executor: public any_executor {
        template_executor(E& executor): _actual_executor{executor} { }
        
        E& _actual_executor;
        
        void post(cf::detail::task_type task) override {
            this->_actual_executor.post(std::move(task));
        }
    };
    
    std::unique_ptr<any_executor> _default;
    
    default_executor() { }
    
public:
    template<typename E>
    static void set(E& executor) {
        get()._default = std::make_unique<template_executor<E>>(template_executor{executor});
    }
    
    static default_executor& get() {
        static default_executor _singleton = {};
        return _singleton;
    }
    
    void post(cf::detail::task_type task) {
        if (_default != nullptr) {
            _default->post(std::move(task));
        } else {
            std::thread{std::move(task)}.detach(); // TODO: make a thread_executor to abstract this
        }
    }
};
...
future<detail::callable_ret_type<F, Args...>> async(F&& f, Args&&... args) {
    return async(default_executor::get(), std::forward<F>(f), std::forward<Args...>(args)...);
}
